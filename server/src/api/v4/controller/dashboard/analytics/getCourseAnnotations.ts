...
| where name == "Annotation.Add" and userType == 0 and courseId == "---"
...
export default async (req, res, next) => {
  try {
    ...
    if (redisResponse !== null) {
      res.status(200);
      res.send({ courseAnnotations: redisResponse });
    } else {
      const course: ICourse = res.locals.videx.course;
      const result = await fetch(
        `https://api.applicationinsights.io/v1/apps/${
          APPLICATION_APPINSIGHTS.ID
        }/query?query=customEvents%7C%20extend%20Pacific%3D(timestamp%20-7h)%2CuserId%3Dtostring(customDimensions.userId)%2C%20courseId%3Dtostring(customDimensions.courseId)%2C%20lessonId%3Dtostring(customDimensions.lessonId)%2C%20userType%3Dtolong(customDimensions.userType)%7C%20where%20timestamp%20%3E%20datetime(2019-05-01)%7C%20where%20name%20%3D%3D%20%22Annotation.Add%22%20and%20userType%20%3D%3D%200%20and%20courseId%20%3D%3D%20%22${course.getId()}%22%7C%20summarize%20count(name)%20%20by%20lessonId`,
        {
          method: 'GET',
          headers: {
      ...
      res.status(200);
      res.send({ courseAnnotations: object });
    }
    return;
  } catch (e) {
